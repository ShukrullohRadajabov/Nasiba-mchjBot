...
//								sendMessage.setReplyMarkup(messengerService.getKeyboard(blocUser));
    ...
    public void adminDeleteGoods(Update update, String type, List<Photo> listName) {
        String text;
    ...
    public void mazgi(Update update, String type, List<Photo> listName) {
        ...
        if (text != null && Objects.equals(text, type)) {
            for (Photo photo : listName) {
                printPic(update, photo, user);
                if(photo.getFileId().equals(photo))
            }



          /*  for (int i = 0; i < listName.size()+1; i++) {
                Photo photo = listName.get(i);
                //Photo photo : listName) {
                if (!listName.get(i).getFileId().equals(listName.get(i + 1).getFileId())) {
                    printPic(update, photo, user);
                    System.out.println(i);
                }else {
                    printPic2(update, photo, user);
                }*/


               /* for (Photo photo1 : listName) {
                    int count1 = 0;
                    for (Photo photo2 : listName) {
                        if (photo1.getFileId().equals(photo2.getFileId())) {
                            count1++;
                        }
                    }
                    integers.add(count1);
...
//            }
    ...
    public void printPic(Update update, Photo photo, User user) {
        try {
//            for (Photo photo : listname) {
...
//            }
    ...
    public void printPic2(Update update, Photo photo, User user) {
        try {
            int count = 0;
            for (Basket basket : user.getBasketList()) {
                if (photo.getProductCode().equals(basket.getProductCode())) {
            ...
            execute(SendPhoto.builder()
                    .chatId(update.getMessage().getChatId())
                    .photo(new InputFile(photo.getFileId()))
                    .replyMarkup(userButton2(photo.getProductCode(), count))
                    .caption(photo.getCaption())
                    .build());
        } catch (TelegramApiException e) {
            e.printStackTrace();
...
//            workWithFile.writeUsers();
//            workWithFile.readerUser();
    ...
    public ReplyKeyboard userButton2(String id, int count) {
        List<List<InlineKeyboardButton>> rowList = new LinkedList<>();
        List<InlineKeyboardButton> row1 = new LinkedList<>();
        InlineKeyboardButton button1 = new InlineKeyboardButton();
        button1.setText(" ➕ ");
