			...
			if (s[0].equals("plus")) {
				...
				if (count > 0) {
					editMessageReplyMarkup.setReplyMarkup((InlineKeyboardMarkup)
							new NasibaMCHJ().userButton(s[1], count));
					for (Basket basket : basketList) {
						if (Objects.equals(basket.getProductCode(), s[1])) {
							for (Basket basket1 : user.getBasketList()) {
								if (basket1.getProductCode().equals(basket.getProductCode())) {
				...
				File file = new File("/home/Buyurtmalar", date + ".xlsx");
//				File file = new File("src/main/resources/Byurtmalar", date + ".xlsx");
				try (FileOutputStream out = new FileOutputStream(file);
					...
					row.createCell(4).setCellValue("Jami");
					int count = 2;
					long total = 0;
					...
					for (Basket basket : client.getBasketList()) {
						...
						string.append(" Code: ").append(basket.getProductCode())
								.append("\n Soni: ").append(basket.getCount())
								.append("\n Narxi: ").append(basket.getPrice())
								.append("\n Jami: ").append(basket.getCount() * basket.getPrice())
								.append("\n------------------------\n");
					...
					row.createCell(0).setCellValue("Umumiy summa: ");
					...
					string.append("Umumiy summa: ").append(total);
					sendMessage.setText("Buyurtmangiz qabul qilindi \uD83D\uDCE6\n" + string);
					sendMessage.setChatId(client.getChatId());
					connection().createStatement().execute("insert into orders (user_id, byurtma_code) VALUES (" + client.getChatId() + ",'" + data + "')");
					new NasibaMCHJ().sendMsg(deleteMessage);
				...
				while (resultSet.next()) {
					File file=new File("/home/Buyurtmalar",resultSet.getString(1)+".xlsx");
//					File file=new File("src/main/resources/Byurtmalar",resultSet.getString(1)+".xlsx");
					SendDocument document=new SendDocument(BOT_ADMIN_ID,new InputFile(file));
                    document.setCaption(" Eski buyurtma \n"+client.getFirstName()+"  Tel. raqam: "+client.getContact());
                    new NasibaMCHJ().sendMsg(document);
				}
				...
				sendMessage.setText("Mijoz faollashtrildi.");
				sendMessage.setChatId(BOT_ADMIN_ID);
				...
				sendMessage.setText("Mijoz bloklandi.");
				sendMessage.setChatId(BOT_ADMIN_ID);
				new NasibaMCHJ().sendMsg(sendMessage);
			}
	...
	public void basketCalbak(User user, Message message, String[] s) {
		...
		if (user.getBasketList().isEmpty()) {
			editMessageText.setText("Sizda hozircha mahsulot yo'q.");
